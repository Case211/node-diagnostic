...
- **tests/integration.sh + CI-джоба** — реальное применение `optimize --all` → сверка значений в ядре → идемпотентность → `rollback` дочиста → `protect`/`nft -c` в privileged-контейнере (Ubuntu 24.04 + Debian 12). То, что smoke (dry-run) не покрывал.
